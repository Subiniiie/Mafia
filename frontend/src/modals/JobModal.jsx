...
function JobModal({ isOpen, openModal }) {
    ...
    // 플레이어의 직업 정보를 받아옴
    // 지금은 없으니까 임시로 설정
    // 직업을 어떻게 받아올거야야야야야야야야야야야
    // 독립운동가 -> 변절자 될 때 어떻게 데이터를 또 가져옴?
    // 매번 서버에서 가지고 오는 거니까 가지고 올 경우는 고려X?
    // 그럼 선택/투표를 서버로 어떻게 보내느냐 고려?
    const jobs = {
        '독립운동가': {
    ...
    // 초기 상태 설정 
    // 직업 정보 입력 받을 때 초기값 바꾸기
    const [ myJob, setMyJob ] = useState('밀정')
    const [ jobDetails, setJobDetails ] = useState(' 밀정은 독립운동가와 첩보원을 전부 제거해야합니다.\n' + ' 매일 밤 독립운동가를 살해할 수 있으며 딱 한 번, 독립운동가 한 명을 변절시킬 수 있습니다.')
    const [ jobImage, setJobImage ] = useState('/jobs/밀정.png')
    ...
    // 서버 연결하면 쓸 코드
    // // 서버에서 직업 정보를 받아오는 함수
    // const fetchJobDetails = async function() {
    //     setLoading(true)
    //     setError('')
    //     try {
    //         // url을 뭐를 써야할까
    //         const response = await fetch("정보를 받을 url")
    //         if (!response.ok) {
    //             throw new Error('Failed to fetch job defatuls')
    //         }
    //         // 플레이어 정보를 받음
    //         const data = await response.json()
    //         setMyJob(data.job)
    //         setJobDetails(jobs[data.job]['설명'])
    //         setJobImage(jobs[data.job]['이미지'])
    //     } catch(error) {
